...
const main = async () => {
    ...
    mappings.forEach((mapping) => {
        ...
        if ((
            STANDARD === "activity" & (
            path.startsWith("//iati-activity") | path.startsWith("//iati-activities") | path.startsWith("//@xml:lang")
            )
        ) | (
            STANDARD === "organisation" & (
            path.startsWith("//iati-organisation") | path.startsWith("//iati-organisations") | path.startsWith("//@xml:lang")
            )
        )){
            ...
            if(condition_nodes.length === 0){
                ...
                conditions.forEach((condition) => {
                    ...
                    if(condition_value_match){
                        condition_value = condition_value_match[0].replace(/'/g, "").replace("= ", "");
                    };
    ...
    const unique_codelists = Array.from(new Set(mapping_list.map((m) => {return(m.codelist)})));

    unique_codelists.forEach((codelist) => {
        const codelist_url = `https://cdn.iatistandard.org/prod-iati-website/reference_downloads/${VERSION.replace(".","")}/codelists/downloads/clv3/json/${LANGUAGE}/${codelist}.json`;
    ...
    const sample_data = await Promise.all(cl_list_promises).then(() => {
        ...
        sample_data.forEach((row, i) => {
            ...
            field_names.forEach((field_name) => {
                ...
                if(mapping_list.map((mapping) => {return(mapping.datastore_name)}).includes(field_name)){
                    let mapping_subset = mapping_list.filter((mapping) => {return(mapping.datastore_name === field_name)});
                    if(mapping_subset.length == 1 & !(Object.keys(mapping_subset[0]).includes("condition_datastore_name"))){
                        // Unconditional
                        const codelist_subset = cl_list.filter((codelist_item) => {return(codelist_item.codelist === mapping_subset[0].codelist)});
                        ...
                        value_codes.forEach((value_code) => {
                            const value_name_comp = codelist_subset.filter((cl_item) => {return cl_item.code == value_code});
                            if(value_name_comp.length > 0){
                                value_names.push(value_name_comp[0].name);
                            }else{
                                value_names.push(value_code);
                                console.log(`Warning: ${value_code} is not in codelist ${mapping_subset[0].codelist}`);
                            };
                        });
                        if(value_codes_is_array){
                        ...
                        // Conditional
                        const conditional_datastore_name = mapping_subset[0].condition_datastore_name;
                        ...
                        cond_compare_values.forEach((cond_compare_value, j) => {
                            mapping_subset = mapping_subset.filter((mapping) => {return(mapping.condition_value == cond_compare_value)});
                            if(mapping_subset.length > 0){ // No codelist for this conditional value if 0
                                const codelist_subset = cl_list.filter((codelist_item) => {return(codelist_item.codelist === mapping_subset[0].codelist)});
                                codelist_subset.push({code: "", name: ""});
                                ...
                                if(original_is_array){
                                    if(!Object.keys(sample_data[i]).includes(new_field_name)){
                                        sample_data[i][new_field_name] = sample_data[i][field_name].slice();
                                    }

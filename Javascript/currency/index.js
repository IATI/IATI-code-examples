...
const main = () => {
    ...
    transactions.forEach((transaction, i) => {
        ...
        let transaction_value_date = fetch_field(transaction, "transaction_value_value_date");
        const transaction_iso_date = fetch_field(transaction, "transaction_transaction_date_iso_date");
        ...
        if(transaction_value !== "" & currency !== "" & transaction_value_date !== ""){
            transaction_value_date = Date.parse(transaction_value_date.substr(0, 10));
            const rate = rates.filter((rate) => {return(
            ...
            if(rate.length > 0){
                ...
                console.log(`Warning: No exchange rate found for ${currency} on ${transaction_value_date}`);
            };
        };
    });
    ...
    budgets.forEach((budget, i) => {
        ...
        let budget_period_start = fetch_field(budget, "budget_period_start_iso_date");
        ...
        if(budget_value !== "" & currency !== "" & budget_value_date !== ""){
            ...
            if(budget_value_date <= rates_date){
                ...
                if(rate.length > 0){
                    ...
                    console.log(`Warning: No exchange rate found for ${currency} on ${budget_value_date}`);
                };
            };
        }else if(budget_value !== "" & currency !== "" & budget_period_start !== "" & budget_period_end !== ""){
            ...
            if(budget_period_end <= rates_date){
                ...
                const rates_to_average = rates.filter((rate) => {return(
                    rate.Currency === currency &
                    rate.Date >= budget_period_start &
                    rate.Date <= budget_period_end
                )}).map((rate) => {return(rate.Rate)});
                if(rates_to_average.length > 0){
                    const average_rate = rates_to_average.reduce((a,b) => a + b, 0) / rates_to_average.length;
